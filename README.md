...
#   [3] Windows GUI → Scoop
#   [4] Cross-platform CLI → Formula + Scoop
#   [5] Cross-platform GUI → Cask + Scoop
```
...
# 发布指定目标（跨平台时常用）
dart run bin/tapster.dart publish -t cask     # macOS 上
...
交互式创建 `.tapster.yaml` 配置文件，支持 5 种项目类型：
...
- `-t, --target`: 指定发布目标（`formula` / `cask` / `scoop`），可多次使用
...
- `-c, --config`: 指定配置文件路径
- `-t, --target`: 指定升级目标（`formula` / `cask` / `scoop`）
...
```bash
# 1. 初始化跨平台配置
tapster init          # 选择 [5] Cross-platform GUI

# 2. macOS 上构建并发布
# ... 构建 macOS .zip ...
tapster upgrade -t cask
tapster publish -t cask
